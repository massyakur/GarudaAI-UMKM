...
> Note: The current NEVADA build uses a FastAPI backend with JWT auth. Legacy references to Clerk/Supabase remain for historical context only.
...
2. **Environment Setup**:
   - Copy `.env.example` to `.env` and set `NEXT_PUBLIC_API_URL` to the FastAPI base
